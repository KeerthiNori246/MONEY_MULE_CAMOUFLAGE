"""
embed_snapshots_combined_drift.py
----------------------------------
Runs the frozen MLP encoder on combined-drift snapshots.
Identical logic to embed_snapshots_drift.py — only folder names change.

Reads:
    graphs/baselines/*_combined_drift_{intensity}.gpickle

Outputs:
    graphs/embeddings/combined_drift_low/YYYY-MM-DD.npy  + meta pkl
    graphs/embeddings/combined_drift_medium/...
    graphs/embeddings/combined_drift_high/...
"""
...
for intensity in INTENSITIES:
    os.makedirs(os.path.join(EMBED_DIR, f"combined_drift_{intensity}"), exist_ok=True)
...
def load_frozen_model():
    ...
    print(f"Frozen MLP loaded  "
          f"(in={config['in_dim']} hidden={config['hidden_dim']} "
          f"out={config['out_dim']})")
    return model, config


# ── feature extraction (identical to all other embed scripts) ─────────────
...
def extract_snapshot_features(G_snap):
    ...
    for node, data in G_snap.nodes(data=True):
        if data.get("node_type") != "transaction":
            ...
            entity_fraud_ratio[node] = (
                sum(1 for nb in tx_nbs
                    if G_snap.nodes[nb].get("fraud", 0) == 1) / len(tx_nbs)
                if tx_nbs else 0.0
            )
...
def embed_combined_drift_baseline(model, intensity: str):
    print(f"\n{'='*55}")
    print(f"  Embedding combined_drift_{intensity}")
    print(f"{'='*55}")

    pattern = os.path.join(BASELINE_DIR,
                           f"*_combined_drift_{intensity}.gpickle")
    paths   = sorted(glob.glob(pattern))

    if not paths:
        raise FileNotFoundError(
            f"No combined_drift_{intensity} baselines found.\n"
            "Run create_combined_drift.py first."
        )

    print(f"  Found {len(paths)} snapshots")
    out_dir        = os.path.join(EMBED_DIR, f"combined_drift_{intensity}")
    saved, skipped = 0, 0

    for path in tqdm(paths, desc=f"Embedding [combined_drift_{intensity}]"):
        date_str = os.path.basename(path).replace(
            f"_combined_drift_{intensity}.gpickle", ""
        )
...
def main():
    print("\n" + "=" * 55)
    print("  EMBEDDING -- COMBINED DRIFT BASELINES")
    print("=" * 55)

    model, _ = load_frozen_model()

    for intensity in INTENSITIES:
        embed_combined_drift_baseline(model, intensity)

    print("\n" + "=" * 55)
    print("  DONE -- Next: run compute_egs_combined_drift.py")
    print("=" * 55)
