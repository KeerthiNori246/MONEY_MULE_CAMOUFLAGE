"""
embed_snapshots_feature_drift.py
---------------------------------
Runs the frozen MLP encoder on feature-drift snapshots.
...
Outputs:
    graphs/embeddings/feature_drift_low/YYYY-MM-DD.npy  + meta pkl
    graphs/embeddings/feature_drift_medium/...
    graphs/embeddings/feature_drift_high/...
"""
...
for intensity in INTENSITIES:
    os.makedirs(os.path.join(EMBED_DIR, f"feature_drift_{intensity}"), exist_ok=True)
...
def load_frozen_model():
    ...
    print(f"Frozen MLP loaded  "
          f"(in={config['in_dim']} hidden={config['hidden_dim']} out={config['out_dim']})")
    return model, config
...
def embed_feature_drift_baseline(model, intensity: str):
    print(f"\n{'='*55}")
    print(f"  Embedding feature_drift_{intensity}")
    print(f"{'='*55}")

    pattern = os.path.join(BASELINE_DIR, f"*_feature_drift_{intensity}.gpickle")
    paths   = sorted(glob.glob(pattern))

    if not paths:
        raise FileNotFoundError(
            f"No feature_drift_{intensity} baselines found.\n"
            "Run create_feature_drift.py first."
        )

    print(f"  Found {len(paths)} snapshots")
    out_dir        = os.path.join(EMBED_DIR, f"feature_drift_{intensity}")
    saved, skipped = 0, 0

    for path in tqdm(paths, desc=f"Embedding [feature_drift_{intensity}]"):
        date_str = os.path.basename(path).replace(
            f"_feature_drift_{intensity}.gpickle", ""
        )
...
def main():
    print("\n" + "=" * 55)
    print("  EMBEDDING -- FEATURE DRIFT BASELINES")
    print("=" * 55)

    model, _ = load_frozen_model()

    for intensity in INTENSITIES:
        embed_feature_drift_baseline(model, intensity)

    print("\n" + "=" * 55)
    print("  DONE -- Next: run compute_egs_feature_drift.py")
    print("=" * 55)
